-- Allow art_style to be NULL for stencil_paint and glitter_reveal orders
ALTER TABLE public.orders
  ALTER COLUMN art_style DROP NOT NULL;

-- Replace create_order_with_coupon to accept product_type and make art_style optional
CREATE OR REPLACE FUNCTION public.create_order_with_coupon(
  ...
  _product_type text DEFAULT 'paint_by_numbers'
)
...
DECLARE
  ...
  v_product_type text;
BEGIN
  v_product_type := coalesce(nullif(trim(_product_type), ''), 'paint_by_numbers');

  v_base_price := CASE _kit_size
    WHEN 'stamp_kit_40x50' THEN 449
    WHEN 'stamp_kit_30x40' THEN 349
    WHEN 'stamp_kit_40x60' THEN 499
    WHEN 'stamp_kit_A4' THEN 249
    WHEN 'stamp_kit_A3' THEN 299
    WHEN 'stamp_kit_A2' THEN 399
    ELSE NULL
  END;
  ...
  -- Art style is only required for paint_by_numbers
  IF v_product_type = 'paint_by_numbers' AND NULLIF(trim(_art_style), '') IS NULL THEN
  ...
  VALUES (
    ...
    _instruction_code,
    _kit_size,
    NULLIF(trim(coalesce(_art_style, '')), ''),
    _photo_url,
...
-- Grant permissions for both old and new signatures
...
REVOKE ALL ON FUNCTION public.create_order_with_coupon(text, text, text, text, text, text, text, boolean, text, text, text, text, text, text, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_order_with_coupon(text, text, text, text, text, text, text, boolean, text, text, text, text, text, text, text, text, text, text, text) TO service_role;
